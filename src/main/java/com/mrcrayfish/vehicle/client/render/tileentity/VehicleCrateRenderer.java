...
public class VehicleCrateRenderer implements BlockEntityRenderer<VehicleCrateBlockEntity>
{
    ...
    public void render(VehicleCrateBlockEntity entity, float delta, @NotNull PoseStack matrices, @NotNull MultiBufferSource buffers, int light, int overlay)
    {
        ...
        matrices.pushPose();
        {
            ...
            if(entity.getEntity() instanceof VehicleEntity vehicle && entity.isOpened())
            {
                matrices.translate(0.5F, 0.0F, 0.5F);

                double progress = Math.min(1.0F, Math.max(0, (entity.getTimer() - 150 + 5 * delta)) / 100.0);
                Pair<Float, Float> scaleAndOffset = EntityRayTracer.instance().getCrateScaleAndOffset((EntityType<? extends VehicleEntity>) vehicle.getType());
                ...
                EntityRenderer<? super VehicleEntity> renderer = Minecraft.getInstance().getEntityRenderDispatcher().getRenderer(vehicle);
                renderer.render(vehicle, 0.0F, delta, matrices, buffers, light);
            }
        }
        matrices.popPose();
    }
